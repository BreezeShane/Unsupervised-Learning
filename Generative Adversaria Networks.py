...
def save_module(D, G):
    if not os.path.exists("GAN_Model"):
        os.mkdir("GAN_Model")
    D_path = os.path.join("GAN_Model", "Discriminator.pt")
    G_path = os.path.join("GAN_Model", "Generator.pt")
    torch.save(D, D_path)
    torch.save(G, G_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--main_epoch", type=int, default=1500)
    parser.add_argument("--D_epoch", type=int, default=5)
    ...
    if D is None or G is None:
        if opt.use_cuda:
            ...
            G = Generator()
            D = Discriminator()
    optim_G = optim.Adam(G.parameters(), lr=opt.D_lr, betas=opt.G_betas)
    optim_D = optim.Adam(D.parameters(), lr=opt.G_lr, betas=opt.D_betas)
